...
import { ApiBearerAuth, ApiNotFoundResponse, ApiOkResponse, ApiOperation, ApiServiceUnavailableResponse, ApiTags, ApiUnauthorizedResponse } from '@nestjs/swagger';
...
export class AuthController {
  ...
  @ApiOkResponse({ description: 'Ingreso al sistema correcto', example: { message: 'string', access_token: 'string' } })
  ...
  @ApiOperation({ summary: 'Petición de envio de codigo' })
  @ApiOkResponse({ description: 'El codigo fue enviado con exito', example: { message: 'string' } })
  @ApiNotFoundResponse({ description: SWAGGER.NOT_FOUND('usuario'), schema: { example: ERROR_CODE.NOT_FOUND('usuario') } })
  @ApiServiceUnavailableResponse({ description: 'No se logró enviar el codigo', schema: { example: ERROR_CODE.SERVICE_UNAVAILABLE('No se puede enviar el codigo, servicio no disponible temporalmente') } })
  @Post('user/code')
  async reqCode(@Body() reqCodeUserDto: ReqCodeUserDto) {
    return this.authService.reqUserCode(reqCodeUserDto)
  }
